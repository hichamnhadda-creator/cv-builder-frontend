...
const ModernTemplate = ({ data, customization }) => {
    ...
    return (
        <div className="bg-white h-full flex flex-col shadow-lg overflow-hidden break-words max-w-full" style={{ fontFamily }} dir={i18n.dir()}>
            ...
            {/* Body Area */}
            <div className={`flex-1 grid grid-cols-1 ${isRtl ? 'md:flex-row-reverse' : 'md:flex-row'} md:grid-cols-3 gap-0 min-h-0`}>
                {/* Main Column (2/3) */}
                <div className={`md:col-span-2 p-8 md:p-10 space-y-10 ${isRtl ? 'border-l' : 'border-r'} border-slate-100 text-start`}>
                    {/* Summary */}
                            ...
                            <div className="space-y-8">
                                {experience.map((exp) => (
                                    <div key={exp.id} className="group">
                                        ...
                                        <p className={`text-slate-600 text-sm leading-relaxed ${isRtl ? 'border-r-2 pr-4' : 'border-l-2 pl-4'} border-slate-50`}>{exp.description}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
                ...
                {/* Sidebar Column (1/3) */}
                <div className="bg-slate-50/50 p-8 md:p-10 space-y-10 text-start">
                    {/* Skills */}
